#!/usr/bin/env python3
"""
RECOVERY PATCH — fix the broken patients.py.

The previous patch script (patch_add_patients_router.py) made three wrong assumptions:
  1. Called next_patient_visible_id() — your actual function is next_visible_id()
  2. Assumed Patient.name was a hybrid property — it's actually name_encrypted (bytes column)
     accessed via encrypt()/decrypt() from app.crypto
  3. Assumed main.py needed a new include_router line — it already had one for patients,
     meaning the original patients.py (with /timeline endpoint) was overwritten

This script:
  - Backs up the broken patients.py
  - Writes a correct version that PRESERVES the existing timeline endpoint AND adds
    the new list + create endpoints
  - Does NOT touch main.py (it's already correct)

Run from your scanguru-portal repo root:

    python3 scripts/patch_patients_recovery.py
...
TARGET = "app/routers/patients.py"

if not os.path.exists(TARGET):
    sys.exit(f"ERROR: {TARGET} not found. Run from your scanguru-portal repo root.")

# Back up whatever's there now (the broken version)
backup = TARGET.replace('.py', f'_broken_backup_{datetime.now().strftime("%Y%m%d_%H%M")}.py')
shutil.copy(TARGET, backup)
print(f"Broken file backed up to {backup}")

CORRECT_CONTENT = '''"""Patient endpoints: list, search, detail, timeline, create."""
import uuid
from datetime import datetime
...
print(f"Wrote corrected {TARGET} ({len(CORRECT_CONTENT)} bytes)")
print()
print("This file now contains:")
print("  GET    /api/v1/patients                — paginated list + search")
print("  POST   /api/v1/patients                — create new patient")
print("  GET    /api/v1/patients/{id}           — patient detail (preserved)")
print("  GET    /api/v1/patients/{id}/timeline  — patient + studies (preserved)")
print()
print("main.py is left untouched — it already imports and registers the router.")
print()
print("Restart uvicorn now:")
print("  Ctrl-C in the uvicorn terminal, then re-run:")
print("  PYTHONPATH=. python -m uvicorn app.main:app --reload --port 8001")
